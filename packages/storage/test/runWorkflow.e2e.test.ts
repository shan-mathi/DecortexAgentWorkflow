// End-to-end integration test: runWorkflow against a real Postgres
// (Testcontainers) plus FakeLLM, exercising every layer end to end.
//
...
//   5. Idempotent appendNodeExecution end-to-end (PostgreSQL primary
//      key + ON CONFLICT DO NOTHING).
//
// Skipped when Docker is absent; otherwise run on every commit.
...
d("runWorkflow E2E (testcontainers)", () => {
  let harness: PgHarness;
  let runRepo: RunRepoPostgres;
  let llm: FakeLLM;
  let runner: QueryRunner;

  const fixturesRoot = join(new URL("../../..", import.meta.url).pathname, "fixtures");
  ...
  it("ops-ticket-router runs end-to-end and persists the full trace", async () => {
    ...
    const r = await runWorkflow(def, { subject, body }, runId, { registry, runRepo });
